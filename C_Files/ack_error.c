...
// Ack Error Example for I2C Peripheral
...
int main(int argc, char **argv)
{
    ...
    uint32_t addr_val     = 0x00000008; // Address -- INCORRECT ADDRESS
    uint32_t reg_val      = 0x00000009; // Register
    uint32_t data_val     = 0x00000066; // Data
    uint32_t byte_count   = 0;          // Byte Count (0 = 1 byte for this implementation)
    uint32_t rw_is_read   = 0;          // R/W
    uint32_t use_register = 1;          // Use Register
    uint32_t use_repeated = 0;          // Use Repeated Start
    uint32_t test_out_en  = 1;          // Test Out
    uint32_t ignore_ack   = 0;          // Ignore ACK -- DISABLED
    ...
    // Step 1: Write to (incorrect) ADDRESS Register
    printf("Writing ADDRESS = 0x%08X\n", addr_val);
    mmio_write32(i2c_ptr, ADDR_OFFSET, addr_val);
    usleep(1000);

    // Step 2: Write to REGISTER Register
    printf("Writing REGISTER = 0x%08X\n", reg_val);
    mmio_write32(i2c_ptr, REG_OFFSET, reg_val);
    usleep(1000);

    // Step 3: Write to DATA Register
    printf("Writing DATA    = 0x%08X\n", data_val);
    mmio_write32(i2c_ptr, DATA_OFFSET, data_val);
    usleep(1000);

    // Step 4: Write to CONTROL Register
    printf("Writing CONTROL (config only) = 0x%08X\n", control_val);
    mmio_write32(i2c_ptr, CONTROL_OFFSET, control_val);
    usleep(1000);

    // Step 5: Trigger the FSM Start
